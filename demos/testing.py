...
phases = ["P", "S", "ScS"]
pairings = product(sources, receivers, phases)
...
class G:
    def __init__(self, model, srpList):
        self.__model = model
        self.__srpList = srpList
    ...
    def __calcMatrix__(self):
        for (source_lat, source_lon, source_depth), (receiver_lat, receiver_lon), phase in self.__srpList:
            self.__computeKernels__(model, source_lat, source_lon, source_depth, receiver_lat, receiver_lon, phase)

        # make sparse matrix of kernels
    ...
    def __computeKernels__(self, model, source_lat, source_lon, source_depth, receiver_lat, receiver_lon, phase):
        print(f"Source: ({source_lat}°, {source_lon}°, {source_depth} km)")
        print(f"Receiver: ({receiver_lat}°, {receiver_lon}°, 0 km)")
        print(f"Phase: {phase}")
